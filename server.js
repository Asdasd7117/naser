...
// ==== إنشاء مجلدات تلقائياً إذا لم توجد ====
const DATA_DIR = path.join(__dirname,'data');
const UPLOADS_DIR = path.join(__dirname,'uploads');
if(!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR);
if(!fs.existsSync(UPLOADS_DIR)) fs.mkdirSync(UPLOADS_DIR);

// ==== إعداد رفع الصور والتواقيع ====
...
function readJSON(file){ if(!fs.existsSync(file)) fs.writeFileSync(file,JSON.stringify([])); return JSON.parse(fs.readFileSync(file)); }
...
app.post('/api/login',(req,res)=>{
  ...
  const found = users.find(u=>u.user===user && u.password===password && u.role===role);
  if(found) res.json({success:true, role, user});
